...
Num<-500
cycle<-data.frame(X=runif(Num,70,135),Y=runif(Num,10,55),Z=runif(Num,20,55))
...
df_China2 <- merge(watershedPoints, dataProjected@data, by = "id")
...
#---------------------------------二维数据插值---------------------------------------------------------
cycle_interp <- interp(cycle$X, cycle$Y, cycle$Z, 
                       xo= seq(60, 140, 0.1), yo= seq(10,60,0.1),
                       linear=FALSE,extrap=TRUE) 

temp=SpatialPoints(expand.grid(x=cycle_interp$x, 
                               y=cycle_interp$y))

temp = SpatialPixelsDataFrame(temp, data.frame(kde = array(cycle_interp$z, 
                                                      length(cycle_interp$z))))
#-------------------------------------------------------------------------------------------------------
